...
def train_with_DSN():
    ...
    for epoch in range(epoch_num):
        for batch_img, _ in train_dataloader:
            # train DSN 可通过skip_step控制训练DSN的频率
            if steps % skip_step == 0:
                ...
                # run MAE decoder
                # 见model_mae: def forward(self, imgs, encoder_res=None, mask_ratio=0.75)
                _, pred_imgs, mask = model_mae(batch_img,
                                               [ouput.detach().cpu(), mask, ids_restore])
                ...
                # first_itreation 用于在训练的第一个迭代中初始化相关变量
                if first_itreation:
