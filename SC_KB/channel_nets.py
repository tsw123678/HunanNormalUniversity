...
def channel_set_gen(num_channels, tap_num, if_toy):
    ...
    for ind_channels in range(num_channels):
        if if_toy:
            ...
            chan_var = 1 / (
                    2 * tap_num)  # since we are generating real and im. part indep. so 1/2 and we are considering complex, -> 2L generated
...
def complex_mul_taps(h, x_tensor):
    ...
    for ind_channel_use in range(x_tensor.shape[1] // 2):  # "//" : 取整除 - 返回商的整数部分（向下取整）
        for ind_conv in range(min(L, ind_channel_use + 1)):
            if len(h.shape) == 1:
                y[:, (ind_channel_use) * 2:(ind_channel_use + 1) * 2] += complex_mul(h[2 * ind_conv:2 * (ind_conv + 1)],
                                                                                     x_tensor[:,
                                                                                     (ind_channel_use - ind_conv) * 2:(
                                                                                                                              ind_channel_use - ind_conv + 1) * 2])
            else:
...
# channel_net不改变形状 由之前的M决定
class channel_net(nn.Module):
    def __init__(self, M=800, num_neurons_encoder=128, n=128,
                 n_inv_filter=2, num_neurons_decoder=128, if_bias=True,
                 if_RTN=False, snr=25, rali=True):
