    ...
    def inference(self, xtype=[], condition=[], condition_types=[], n_samples=1, mix_weight={'audio': 1, 'text': 1, 'image': 1}, image_size=256, ddim_steps=50, scale=7.5, num_frames=8):
        ...
        for i, condition_type in enumerate(condition_types):
            if condition_type == 'image':
                ctemp1 = regularize_image(condition[i]).to('cpu')
                ctemp1 = ctemp1[None].repeat(n_samples, 1, 1, 1)
                cim = net.clip_encode_vision(ctemp1).to('cpu')
                uim = None
                if scale != 1.0:
                    dummy = torch.zeros_like(ctemp1).to('cpu')
                    uim = net.clip_encode_vision(dummy).to('cpu')
                ...
                ctx = net.clip_encode_text(n_samples * [condition[i]]).to('cpu')
                utx = None
                if scale != 1.0:
                    utx = net.clip_encode_text(n_samples * [""]).to('cpu')
